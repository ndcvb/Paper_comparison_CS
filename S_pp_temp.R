...
cs_data = cs_data %>% select('Watercontactsite',latitude='X_Take.a.GPS.point_latitude',longitude='X_Take.a.GPS.point_longitude',date='today','ID',bio = 'What.is.the.number.of.Biomphalaria.specimens...example.shown.below.',time='Enter.the.ending.time')

# Expert data

ex_data = read.csv("~/Library/CloudStorage/OneDrive-KULeuven/Uganda_Congo/Data/Uganda/Clean/D_ex_filtered.csv", sep = ",", header = TRUE)

ex_data = ex_data %>% select('Watercontactsite',Site.type, date='Date_J',bio = 'Biomphalaria_J',time='S.time_J')
...
cs_data <- cs_data %>% select(-oneweek)
ex_data <- ex_data %>% select(-oneweek)
